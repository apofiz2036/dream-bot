...
async def init_db():
    ...
    - divinations: хранит историю трактований сновидений (user_id, date)
    """
    try:
    ...
    try:
        ...
        if "public_id" not in columns:
            ...
            for (user_id,) in users:
                public_id = f"SLEEP-{secrets.token_hex(3).upper()}"  # Например, SLEEP-A1B2C3
                await cursor.execute(
...
async def save_subscriber(user_id: int):
    ...
    try:
        ...
        if not exist:
            ...
            public_id = f"SLEEP-{secrets.token_hex(3).upper()}"
            await cursor.execute(
...
async def save_divination(user_id: int):
    """Сохраняет информацию о трактовании в базу данных."""
    try:
        conn = await aiosqlite.connect(SQLITE_DB)
        cursor = await conn.cursor()
        ...
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        await cursor.execute(
            "INSERT INTO divinations (user_id, date) VALUES (?, ?)",
            (user_id, timestamp)
        )
