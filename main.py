...
async def handle_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if text == "Трактовать сон":
            await guess_mode(update, context)
        elif text == "Пополнить лимиты":
            await payment_message(update, context)
        elif text == "Как трактовать":
            with open('text/how_to_dream.txt', 'r', encoding='utf-8') as file:
                text = file.read()      
...
def setup_handlers(application) -> None:
    ...
    try:
        ...
        menu_filters = filters.TEXT & (
            filters.Regex("^Трактовать сон$") |
            filters.Regex("^Пополнить лимиты$") |
            filters.Regex("^Как трактовать$") |
